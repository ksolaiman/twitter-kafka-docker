...
screen_names = data['tag'][0]

tagsToSearch = ' OR '.join(screen_names)
...
i=0
# Get User Timelines - statuses/user_timeline.json, param screen_name
# https://developer.twitter.com/en/docs/tweets/timelines/api-reference/get-statuses-user_timeline.html
# 200 is the limit API can read at a time from timeline
for name in screen_names:
	...
	for i in range(num_of_tweets):
		...
		if count == 1:
			break
		status_cursor = tweepy.Cursor(api.user_timeline, screen_name=name, count=200, max_id=min_id,
									  tweet_mode='extended')
		status_list = status_cursor.iterator.next()
